...
namespace ACT4
{
    public partial class Form1 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (temperature > 0.1)
            {
                Point nextMove = (Point)chosenMove;
                ...
                int nextHeuristic = getAttackingPairs(nextState);
                int deltaHeuristic = nextHeuristic - currentHeuristic;

                if (deltaHeuristic < 0 || new Random().NextDouble() < Math.Exp(-deltaHeuristic / temperature))
                {

                    executeMove(nextMove);
                    currentState = new SixState(nextState);
                }
